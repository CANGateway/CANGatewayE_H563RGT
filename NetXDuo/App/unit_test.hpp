...
void can_test() {
    ...
    can1.attach([&](const stmbed::CANMessage &msg) { printf("can1 recv: id: %x\n", msg.id); });

    can2.attach([&](const stmbed::CANMessage &msg) { printf("can2 recv: id: %x\n", msg.id); });
...
void thread_can_test() {
    constexpr size_t STACK_SIZE = 1024;

    stmbed::CAN can1(&hfdcan1);
    stmbed::CAN can2(&hfdcan2);

    std::unique_ptr<threadx::static_thread<STACK_SIZE>> thread1_;
    std::unique_ptr<threadx::static_thread<STACK_SIZE>> thread2_;

    uint8_t cnt = 0;

    thread1_ = std::make_unique<threadx::static_thread<STACK_SIZE>>(
        "Thread1",
        [&](ULONG input) {
            can1.attach([&](const stmbed::CANMessage &msg) { printf("can1 recv: id: %x\n", msg.id); });
            while (1) {
    ...
    thread2_ = std::make_unique<threadx::static_thread<STACK_SIZE>>(
        "Thread2",
        [&](ULONG input) {
            can2.attach([&](const stmbed::CANMessage &msg) { printf("can2 recv: id: %x\n", msg.id); });
            while (1) {
