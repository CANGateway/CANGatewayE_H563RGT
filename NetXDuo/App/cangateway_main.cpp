...
void can_init(FDCAN_HandleTypeDef *handle, uint32_t id_type, uint32_t filter_id, uint32_t filter_mask, uint32_t fifo_ch);
...
extern "C" VOID cangateway_main(ULONG thread_input)
{
    ...
    can_init(&hfdcan1, FDCAN_STANDARD_ID, 0, 0, FDCAN_IT_RX_FIFO0_NEW_MESSAGE);
    can_init(&hfdcan2, FDCAN_STANDARD_ID, 0, 0, FDCAN_IT_RX_FIFO0_NEW_MESSAGE);
...
void receive_thread_entry(ULONG thread_input)
{
    ...
    while (1)
    {
        ...
        if (ret == NX_SUCCESS)
        {
            ...
            CANMessageWithCh message;
            UCHAR *data_buffer_ptr = data_buffer;
            while (1)
            {
                DecodeResult dec_ret = decode_can_message(data_buffer_ptr, bytes_read, message, &decoded_length);
                // printf("%ld\n", bytes_read);
                if (dec_ret == DECODE_SUCCESS)
                {
                    HAL_StatusTypeDef hal_ret = can_write(message, true);
                    // printf("hal_ret: %d\n", hal_ret);
                    // printf("Decoded CAN message: ID: %08lX, DLC: %d, Data: ", message.id, message.dlc);
...
void can_init(FDCAN_HandleTypeDef *handle, uint32_t id_type, uint32_t filter_id, uint32_t filter_mask, uint32_t fifo_ch)
{
    if (handle->State == HAL_FDCAN_STATE_READY)
    {
        ...
        if (HAL_FDCAN_ActivateNotification(handle, fifo_ch, 0) != HAL_OK)
        {
            Error_Handler();
        }
...
//     if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO1, &rx_header, msg.data) == HAL_OK) {
